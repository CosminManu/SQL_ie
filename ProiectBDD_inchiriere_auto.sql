...
DROP TABLE SEDII_P CASCADE CONSTRAINTS;
DROP TABLE ANGAJATI CASCADE CONSTRAINTS;
DROP TABLE ANGAJATI_P CASCADE CONSTRAINTS;
DROP TABLE CLIENTI CASCADE CONSTRAINTS;
DROP TABLE CLIENTI_P CASCADE CONSTRAINTS;
DROP TABLE CONTRACTE CASCADE CONSTRAINTS;
DROP TABLE CONTRACTE_P CASCADE CONSTRAINTS;
DROP TABLE MASINI CASCADE CONSTRAINTS;
DROP TABLE MASINI_P CASCADE CONSTRAINTS;
DROP TABLE ASIGURARI CASCADE CONSTRAINTS;
DROP TABLE ASIGURARI_P CASCADE CONSTRAINTS;
DROP TABLE EVIDENTE_CONTABILE CASCADE CONSTRAINTS;
DROP TABLE EVIDENTE_CONTABILE_P CASCADE CONSTRAINTS;

create table SEDII_P
(
    id_sediu    NUMBER(5),
    adresa      VARCHAR2(40),
...
alter table SEDII_P add constraint SEDII_ID_SED_PK primary key (id_sediu);
alter table SEDII_P add constraint SEDII_ID_SED_CH check (id_sediu > 0);

create table ANGAJATI_P
(
...
alter table ANGAJATI_P add constraint ANGAJATI_ID_ANG_PK primary key (id_angajat);
alter table ANGAJATI_P add constraint ANGAJATI_ID_SED_FK foreign key (id_sediu) references SEDII_P(id_sediu);
alter table ANGAJATI_P add constraint ANGAJATI_EMAIL_TEL_UK unique (email, telefon);
alter table ANGAJATI_P add constraint ANGAJATI_ID_ANG_NN check (id_angajat is not null);
alter table ANGAJATI_P add constraint ANGAJATI_NUME_NN check (nume is not null);
alter table ANGAJATI_P add constraint ANGAJATI_PRENUME_NN check (prenume is not null);
alter table ANGAJATI_P add constraint ANGAJATI_SALARIUL_MIN check (salariul > 0);

create table EVIDENTE_CONTABILE_P
(
...
alter table EVIDENTE_CONTABILE_P add constraint EVIDENTE_ID_EVID_PK primary key (id_evidenta);
alter table EVIDENTE_CONTABILE_P add constraint EVIDENTE_ID_SED_FK foreign key (id_sediu) references  SEDII_P(id_sediu);
alter table EVIDENTE_CONTABILE_P add constraint EVIDENTE_ID_EVID_NN check(id_evidenta is not null);
alter table EVIDENTE_CONTABILE_P add constraint EVIDENTE_VENIT_CH check(venituri > 0);

create table ASIGURARI_P
(
...
alter table ASIGURARI_P add constraint ASIGURARI_ID_ASIG_PK primary key (id_asigurare);
alter table ASIGURARI_P add constraint ASIGURARI_ID_ASIG_NN check(id_asigurare is not null);
alter table ASIGURARI_P add constraint ASIGURARI_PRET_CH check (pret > 0);

create table MASINI_P
(
...
alter table MASINI_P add constraint MASINI_ID_MAS_PK primary key (id_masina);
alter table MASINI_P add constraint MASINI_ID_SED_FK foreign key (id_sediu) references SEDII_P(id_sediu);
alter table MASINI_P add constraint MASINI_ID_ASIG_FK foreign key (id_asigurare) references ASIGURARI_P(id_asigurare);
alter table MASINI_P add constraint MASINI_MARCA_NN check(marca is not null);
alter table MASINI_P add constraint MASINI_SCAUNE_CH check(nr_scaune > 0);
alter table MASINI_P add constraint MASINI_USI_CH check(nr_usi > 0);
alter table MASINI_P add constraint MASINI_KM_CH check(nr_kilometri > 0);
alter table MASINI_P add constraint MASINI_TRANSM_CH check(transmisie in ('A','M'));
alter table MASINI_P add constraint MASINI_CONSUM_CH check(consum in ('benzina','motorina','electric'));
...
alter table CLIENTI_P add constraint CLIENTI_ID_CLI_PK primary key (id_client);
alter table CLIENTI_P add constraint CLIENTI_ID_CLI_NN check (id_client is not null);
alter table CLIENTI_P add constraint CLIENTI_EMAIL_TEL_UK unique (email_client, telefon_client);
alter table CLIENTI_P add constraint CLIENTI_DATA_NAS_NN check(data_nastere is not null);

create table CONTRACTE_P
(
...
alter table CONTRACTE_P add constraint CONTRACTE_ID_CON_PK primary key (id_contract);
alter table CONTRACTE_P add constraint CONTRACTE_ID_MAS_FK foreign key (id_masina) references MASINI_P(id_masina);
alter table CONTRACTE_P add constraint CONTRACTE_ID_CLI_FK foreign key (id_client) references CLIENTI_P(id_client);
alter table CONTRACTE_P add constraint CONTRACTE_ID_CON_NN check(id_contract is not null);
alter table CONTRACTE_P add constraint CONTRACTE_DATA_PREL_NN check(data_preluarii is not null);
alter table CONTRACTE_P add constraint CONTRACTE_DATA_ADUC_NN check(data_aducerii is not null);
alter table CONTRACTE_P add constraint CONTRACTE_DATA_CH check(data_aducerii > data_preluarii);
alter table CONTRACTE_P add constraint CONTRACTE_PRET_CH check (pret > 0);


insert into  SEDII_P (id_sediu, adresa, cod_postal, oras)
...
insert into ANGAJATI_P (id_angajat,prenume,nume,email,telefon,data_angajare,salariul,id_sediu)
...
insert into EVIDENTE_CONTABILE_P (id_evidenta,venituri,cheltuieli,sold_final,id_sediu)
...
insert into ASIGURARI_P (id_asigurare,tip_asigurare,detalii_asigurare,pret)
...
insert into MASINI_P (id_masina,marca,tip,nr_scaune,nr_usi,consum,transmisie,nr_kilometri,status,id_sediu,id_asigurare)
...
insert into CLIENTI_P(id_client,prenume_client,nume_client,telefon_client,email_client,data_nastere,tara_provenienta)
...
insert into CONTRACTE_P (id_contract,pret,cantitate,data_preluarii,data_aducerii,locatia_preluarii,perioada,id_masina,id_client)
